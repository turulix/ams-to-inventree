/*
 * InvenTree API
 *
 * API for InvenTree - the intuitive open source inventory management system
 *
 * The version of the OpenAPI document: 453
 * 
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

/// GlobalSettings : Serializer for the InvenTreeSetting model.
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct GlobalSettings {
    #[serde(rename = "pk")]
    pub pk: i32,
    #[serde(rename = "key")]
    pub key: String,
    #[serde(rename = "value", deserialize_with = "Option::deserialize")]
    pub value: Option<String>,
    #[serde(rename = "name")]
    pub name: String,
    #[serde(rename = "description")]
    pub description: String,
    #[serde(rename = "type")]
    pub r#type: String,
    #[serde(rename = "units")]
    pub units: String,
    /// Returns the choices available for a given item.
    #[serde(rename = "choices")]
    pub choices: Vec<serde_json::Value>,
    #[serde(rename = "model_name", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub model_name: Option<Option<String>>,
    #[serde(rename = "api_url", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub api_url: Option<Option<String>>,
    #[serde(rename = "typ")]
    pub typ: String,
    /// Indicates if the setting is overridden by an environment variable
    #[serde(rename = "read_only")]
    pub read_only: bool,
    /// Indicates if changing this setting requires confirmation
    #[serde(rename = "confirm")]
    pub confirm: bool,
    #[serde(rename = "confirm_text")]
    pub confirm_text: String,
}

impl GlobalSettings {
    /// Serializer for the InvenTreeSetting model.
    pub fn new(pk: i32, key: String, value: Option<String>, name: String, description: String, r#type: String, units: String, choices: Vec<serde_json::Value>, typ: String, read_only: bool, confirm: bool, confirm_text: String) -> GlobalSettings {
        GlobalSettings {
            pk,
            key,
            value,
            name,
            description,
            r#type,
            units,
            choices,
            model_name: None,
            api_url: None,
            typ,
            read_only,
            confirm,
            confirm_text,
        }
    }
}

